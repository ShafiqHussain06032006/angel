...
export class SidebarProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'angel.agentPanel';
        ...
        webviewView.webview.onDidReceiveMessage(async (message) => {
            ...
            switch (data.type) {
                case 'processInputSubmit':
                    await vscode.commands.executeCommand('angel.processInputWithData', data.apiKey, data.input, data.mode, data.provider, data.model);
                    break;
                case 'start-sdlc':
                    await vscode.commands.executeCommand('angel.startSDLC', data.input, data.apiKey, data.provider);
                    break;
                case 'newTask':
                    await vscode.commands.executeCommand('angel.newTask');
                    break;
                case 'openFeedback':
                    vscode.env.openExternal(vscode.Uri.parse('https://github.com/maybeanns/Angel/issues'));
                    break;
                case 'listConversations':
                    await vscode.commands.executeCommand('angel.listConversations');
                    break;
                case 'loadConversation':
                    await vscode.commands.executeCommand('angel.loadConversation', data.conversationId);
                    break;
                case 'deleteConversation':
                    await vscode.commands.executeCommand('angel.deleteConversation', data.conversationId);
                    break;
                case 'mcpInstall':
                    await vscode.commands.executeCommand('angel.mcpInstall', data.serverId, data.scope || 'project');
                    break;
                case 'triggerUpload':
                ...
                case 'voiceConversationComplete':
                    ...
                    await vscode.commands.executeCommand('angel.voiceConversationComplete', data.summary, data.transcript);
                    break;
                ...
                case 'log':
                    ...
                    break;
                case 'showOutput':
                    await vscode.commands.executeCommand('angel.showOutput');
                    break;
                case 'saveApiKey':
                    await vscode.commands.executeCommand('angel.saveApiKey', data.provider, data.apiKey);
                    webviewView.webview.postMessage({ type: 'apiKeySaved', provider: data.provider });
                    break;
                case 'deleteApiKey':
                    await vscode.commands.executeCommand('angel.deleteApiKey', data.provider);
                ...
                case 'webviewReady':
                    ...
                    await vscode.commands.executeCommand('angel.ensureLLMReady');
                    break;
            }
        });
    ...
    private async processAudioBase64(base64Audio: string, webviewView: vscode.WebviewView) {
        ...
        // ConfigService stores under 'angel.apikey.groq' — use the same prefix here
        let groqKey = (await this.context.secrets.get('angel.apikey.groq')) || this.sessionVoiceKey;
