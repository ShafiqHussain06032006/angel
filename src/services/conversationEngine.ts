/**
 * ConversationEngine — SDLC-aware conversational AI for Angel.
 *
 * Responsibilities:
...
\`\`\``
        ...
        return `You are Angel, an SDLC-aware AI coding assistant with expert knowledge of software architecture, security (OWASP Top 10), clean code, testing, DevOps, and project planning.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━`;
    ...
    private async thinkFast(userSpeech: string, snapshot: WorkspaceSnapshot): Promise<string> {
        ...
        const systemPrompt = `You are Angel, a conversational AI pair programmer.
...
Stack context: ${snapshot.stackSummary}.`;
    ...
    private async callLLM(systemPrompt: string, maxTokens: number): Promise<string> {
        ...
        return "No API keys configured. Please set your Anthropic or Groq key via the Angel: Set API Key command.";
    }
    ...
    private async callAnthropic(key: string, systemPrompt: string, maxTokens: number): Promise<string> {
        try {
            ...
            return `I ran into an error calling Anthropic. Check the Angel output log.`;
        }
    }
    ...
    private async callGroq(key: string, systemPrompt: string, maxTokens: number): Promise<string> {
        try {
            ...
            return `I ran into an error calling Groq. Check the Angel output log.`;
        }
    }
