...
export async function transcribeAudio(
    ...
    const config = vscode.workspace.getConfiguration('angel');
    ...
    if (!groqProvider) {
        // Keys are stored in SecretStorage (ConfigService) — not in plaintext VS Code settings.
        // At this layer we only have access to env vars as a last resort.
        // The caller (SidebarProvider / extension.ts) should call setGroqProviderInstance()
        // after initializing the provider via ConfigService.
        const groqKey = process.env.GROQ_API_KEY;
        if (!groqKey) {
            throw new Error(
                'No Groq API key available for cloud transcription. ' +
                'Please add your Groq API key via the Angel sidebar profile button.'
            );
        }
