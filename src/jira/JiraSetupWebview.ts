...
export class JiraSetupWebview {
    ...
    private async handleSelectProject(projectKey: string) {
        try {
            ...
            // Save config to .angel/jira-config.json
            const wsRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
            if (wsRoot) {
                const configPath = path.join(wsRoot, '.angel', 'jira-config.json');
                const angelDir = path.dirname(configPath);
                if (!fs.existsSync(angelDir)) fs.mkdirSync(angelDir, { recursive: true });
...
<body>
    ...
    <div id="step1">
        <p>Connect Angel to your Jira instance to auto-push SDLC tasks.</p>
