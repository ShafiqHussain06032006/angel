...
export class SDLCWebviewPanel {
    ...
    private artifacts: AngelArtifactService | null = null;
    ...
    private constructor(panel: vscode.WebviewPanel, context: vscode.ExtensionContext, logger: Logger, llmService: LLMService) {
        ...
        if (root) { this.artifacts = new AngelArtifactService(root); }
            ...
            switch (msg.type) {
                ...
                case 'complete-flow':
                    ...
                    if (this.state.prdDocument) {
                        await vscode.commands.executeCommand('angel.startBMADAfterSDLC', this.state.prdDocument);
                    }
                    break;
                case 'load-state':
                    this.loadState();
    ...
    public static createOrShow(context: vscode.ExtensionContext, logger: Logger, llmService: LLMService, topic?: string, prd?: PRDDocument) {
        ...
        const panel = vscode.window.createWebviewPanel('sdlcFlow', 'Angel — PRD Review', vscode.ViewColumn.Beside, { enableScripts: true });
    ...
    private loadState() {
        const root = this.getWorkspaceRoot();
        if (root) {
            if (!this.artifacts) { this.artifacts = new AngelArtifactService(root); }
...
[
  {
    ...
    "stories": [
      {
        ...
        "subtasks": [
          { "id": "st1", "title": "Subtask title", "status": "To Do", "syncStatus": "pending", "parentStoryId": "s1", "assignedAgent": "developer" }
            ...
            vscode.window.showErrorMessage(`Angel Task Generation Error: ${msg}. Check logs and retry.`);
    ...
    private async pushToJira(dryRun: boolean) {
        ...
        let config: any = {};
        try {
            config = JSON.parse(fs.readFileSync(path.join(root, '.angel', 'jira-config.json'), 'utf-8'));
        } catch (e) {
