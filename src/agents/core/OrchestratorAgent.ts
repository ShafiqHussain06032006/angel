 ...
 *
 * Plan state is persisted in .angel/plan-state/plan.json between invocations.
 * Old plans are backed up with timestamps in .angel/plan-state/history/.
 */
...
import { AngelArtifactService } from '../../services/artifact/AngelArtifactService';
...
export class OrchestratorAgent extends BaseAgent {
  ...
  private initializeEnhancedServices(workspaceRoot: string): void {
    if (!workspaceRoot) {
      this.log('No workspace folder open. Enhanced orchestrator services are disabled.');
      return;
    }
    if (!this.todoService) {
  ...
  public async startSDLCFlow(topic: string): Promise<void> {
    await vscode.commands.executeCommand('angel.startSDLC', topic);
  }
  ...
  public async onPRDApproved(prd: PRDDocument, context: vscode.ExtensionContext, agentPanel?: any, cancellationToken?: vscode.CancellationToken): Promise<void> {
    ...
    try {
        ...
        if (agentPanel) {
            ...
            try {
                const logDir = require('path').join(workspaceRoot, '.angel', 'sdlc-logs');
        ...
        if (agentPanel) {
            ...
            try {
                ...
                const ignoreDirs = new Set(['.git', '.angel', '.planning', 'node_modules', '.vscode', 'dist', 'build', 'out', 'coverage']);
            ...
            if (agentSummary) {
                ...
                summaryMsg += '> Full agent outputs saved to `.angel/sdlc-logs/`\n\n';
            }
    ...
    try {
      ...
      for (const step of plan.steps) {
        ...
        try {
          ...
          // Save full output to .angel/sdlc-logs/ instead of dumping in chat
          if (context.workspaceRoot) {
              try {
                  const logDir = require('path').join(context.workspaceRoot, '.angel', 'sdlc-logs');
                  if (!require('fs').existsSync(logDir)) require('fs').mkdirSync(logDir, { recursive: true });
  ...
  private async runPendingCodingAgents(
    ...
    for (const step of pendingCodingSteps) {
      ...
      if (step.agentId === 'developer' && context.workspaceRoot) {
        try {
          const artifacts = new AngelArtifactService(context.workspaceRoot);
      ...
      try {
        ...
        // Save full output to .angel/sdlc-logs/
        if (context.workspaceRoot) {
            try {
                const logDir = require('path').join(context.workspaceRoot, '.angel', 'sdlc-logs');
                if (!require('fs').existsSync(logDir)) require('fs').mkdirSync(logDir, { recursive: true });
