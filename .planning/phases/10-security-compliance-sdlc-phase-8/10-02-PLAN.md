...
- File contains `ANGEL_SECRET_SCANNER` in `buildHookScript()`
- `tsc --noEmit` passes
</acceptance_criteria>
...
- File contains `angel.scanForSecrets`
- File contains `angel.installSecretScanner`
- File contains `angel.uninstallSecretScanner`
...
3. **Add contributions** to `package.json` under `"contributes"."commands"`:
   ```json
   {
     "command": "angel.scanForSecrets",
     "title": "Angel: Scan for Secrets",
     "category": "Angel"
   },
   {
     "command": "angel.installSecretScanner",
     "title": "Angel: Install Pre-Commit Secret Scanner",
     "category": "Angel"
   },
   {
     "command": "angel.uninstallSecretScanner",
     "title": "Angel: Uninstall Secret Scanner Hook",
     "category": "Angel"
   }
   ```
</action>
...
- `package.json` contains `"angel.scanForSecrets"`
- `package.json` contains `"angel.installSecretScanner"`
- `package.json` contains `"angel.uninstallSecretScanner"`
...
- Status bar shows `$(shield) Scanner Off` initially; changes to `$(shield) Angel Scanner Active` after `angel.installSecretScanner`
</acceptance_criteria>
...
# Confirm ANGEL_SECRET_SCANNER marker in hook script
Select-String -Path src/services/project/SecretScannerService.ts -Pattern "ANGEL_SECRET_SCANNER"

# Confirm all 5 commands in package.json
Select-String -Path package.json -Pattern "angel.scanForSecrets"
Select-String -Path package.json -Pattern "angel.installSecretScanner"
Select-String -Path package.json -Pattern "angel.uninstallSecretScanner"
Select-String -Path package.json -Pattern "angel.generateOwaspChecklist"
Select-String -Path package.json -Pattern "angel.checkCompliance"
```

### Manual Verification
1. Plant a test secret in a temp file: `# TEST: AKIAIOSFODNN7EXAMPLE`
2. Run `angel.scanForSecrets` → finding appears in Angel Security output channel at correct file+line
3. Remove temp file
4. Run `angel.installSecretScanner` → `.git/hooks/pre-commit` exists; status bar changes to active
5. Stage a file with a planted secret → `git commit` blocked with Angel message
6. Run `angel.uninstallSecretScanner` → hook removed; status bar changes back
...
- [ ] `installPreCommitHook()` writes `.git/hooks/pre-commit` with ANGEL_SECRET_SCANNER marker
