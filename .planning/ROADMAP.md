...
- [ ] **Phase 13: Agent Test Coverage & CI for Angel** — MockLLMService, unit tests, GitHub Actions for extension itself
...
**Success Criteria** (what must be TRUE):
  ...
  4. Agent artifacts write to `.angel/` only — not workspace root
...
**Success Criteria** (what must be TRUE):
  1. Angel detects if workspace is a GitHub repo (via `git remote`)
...
- [x] 10-02: Pre-Commit Secret Scanner — git hook injection + Angel command
...
**Success Criteria** (what must be TRUE):
  1. On file save, Angel detects if a READMEsection is stale and offers to regenerate
  2. `angel.generateChangelog` command produces CHANGELOG.md from git history
...
### Phase 13: Agent Test Coverage & CI for Angel
**Goal**: Establish injectable MockLLMService, write agent/service unit tests, add GitHub Actions CI for the Angel extension itself.
**Depends on**: Phase 12
**Requirements**: TST-04
**Success Criteria** (what must be TRUE):
  ...
  3. GitHub Actions CI runs lint → compile → test on every push to Angel's own repo
**Plans**: 2 plans

Plans:
- [x] 13-01: MockLLMService + Agent Unit Tests — injectable mock, critical agent tests
- [x] 13-02: Angel CI Pipeline — GitHub Actions for extension lint + compile + test
...
| 13. Agent Test Coverage & Angel CI | 2/2 | Completed | 2026-04-15 |
